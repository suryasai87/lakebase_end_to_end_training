...
def initialize_database():
    """Initialize database schema, tables, audit trail, triggers, and sample data on app startup."""
    print("Checking database initialization...")
    try:
        with LakebaseConnection() as db:
            # Check if schema exists
            db.cursor.execute("SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'ecommerce'")
            schema_exists = db.cursor.fetchone() is not None

            if not schema_exists:
                print("Creating ecommerce schema...")
                db.cursor.execute("CREATE SCHEMA IF NOT EXISTS ecommerce")
            ...
            """)
            users_exists = db.cursor.fetchone() is not None
            ...
            """)
            audit_exists = db.cursor.fetchone() is not None
            ...
            if not audit_exists:
                ...
                print("Audit log table and indexes created")
            ...
            """)
            trigger_count = db.cursor.fetchone()['count'] if db.cursor.fetchone() else 0

            # Create trigger functions and triggers
            print("Setting up audit triggers...")

            # Products audit trigger function
            db.cursor.execute("""
            ...
            # Order items audit trigger function
            db.cursor.execute("""
            ...
            for trig in triggers:
                try:
                    db.cursor.execute(trig)
                    db.connection.commit()
                except Exception as e:
                    print(f"Trigger warning: {str(e)[:80]}")
                    db.connection.rollback()

            print("Audit triggers configured")

            # Check if sample data exists
            db.cursor.execute("SELECT COUNT(*) as count FROM ecommerce.users")
            ...
            if user_count == 0:
                ...
                db.cursor.execute("SELECT user_id FROM ecommerce.users LIMIT 1")
            ...
            print(f"Database initialized: Users={result['users']}, Products={result['products']}, Orders={result['orders']}, Audit entries={result['audit_entries']}")
            return True
